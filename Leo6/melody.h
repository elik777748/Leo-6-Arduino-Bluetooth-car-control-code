...
const Song songs[] = {
    { melody1, sizeof(melody1)/sizeof(melody1[0])/2, 140, 0.7, true},
    { melody2, sizeof(melody2)/sizeof(melody2[0])/2, 120, 1.0, true},
    { melody3, sizeof(melody3)/sizeof(melody3[0])/2, 120, 1.0, true},
    { melody4, sizeof(melody4)/sizeof(melody4[0])/2, 120, 1.0, true},
    { melody5, sizeof(melody5)/sizeof(melody5[0])/2, 120, 1.0, true}
};
...
static unsigned long noteTimer = 0;

inline void startSong(uint8_t id) {
    if (id >= SONG_COUNT) return;
    currentSong = id;
    playing = true;
    noteIndex = 0;
    noteOn = false;
...
inline void updateSong() {
    if (!playing) return;

    const Song& s = songs[currentSong];

    if (noteIndex >= s.length) {
        if (s.infinite) noteIndex = 0;
        else {
            stopAllSounds();
            return;
        }
    ...
    int16_t note = s.melody[noteIndex * 2];
    int16_t duration = s.melody[noteIndex * 2 + 1];
    unsigned long durMs = (60000UL * 4 / s.tempo) / abs(duration) * s.tempoMultiplier;

    if (!noteOn) {
        if (note != REST) tone(buzzerPin, note);
        noteOn = true;
        noteTimer = now;
    }

    if (noteOn && now - noteTimer >= durMs * 0.9) noTone(buzzerPin);
    if (now - noteTimer >= durMs) {
        noteOn = false;
        noteIndex++;
    }
}
